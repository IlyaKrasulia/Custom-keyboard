...
export const CustomKeyboard = ({
  ...
  const onPressKey = useCallback((key) => {
    setSelection((prevSelection) => {
      setValue((prevVal) => `${prevVal.slice(0, prevSelection.start)}${key}${prevVal.slice(prevSelection.start)}`);
  ...
  const onPressSpace = useCallback(() => {
    setSelection((prevSelection) => {
      setValue((prevVal) => `${prevVal.slice(0, prevSelection.start)} ${prevVal.slice(prevSelection.start)}`);
      return { start: prevSelection.start + 1, end: prevSelection.start + 1 };
    });
  }, [setSelection, setValue]);
    ...
    setSelection((prevSelection) => {
      if (prevSelection.start > 0) {
        if (prevSelection.start !== prevSelection.end) {
          setValue((prevVal) => `${prevVal.slice(0,prevSelection.start)}${prevVal.slice(prevSelection.end, prevVal.length)}`);
          return { start: prevSelection.start, end: prevSelection.start };
        } else {
          setValue((prevVal) => `${prevVal.slice(0, prevSelection.start - 1)}${prevVal.slice(prevSelection.start)}`);
          return {
